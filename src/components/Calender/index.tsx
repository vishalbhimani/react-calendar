					...
					{rows.map((row, rowIndex) => (
						<tr key={rowIndex}>
							{row?.map((day, dayIndex) => {
								let className = day === currentDay() && dateContext.format('MM') === moment().format('MM') ? 'day current-day' : 'day';
								return day === 0 ? (
									<td />
								) : (
									<DateBlock
										...
										className={className}
										label={day.toString()}
										labelMonth={dateContext.format('MM')}
										labelYear={dateContext.format('YYYY')}
